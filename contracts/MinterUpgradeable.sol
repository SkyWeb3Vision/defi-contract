...
contract MinterUpgradeable is IMinter, OwnableUpgradeable {
    ...
    uint public weekly; // represents a starting weekly emission of 2.6M SKY (SKY has 18 decimals)
    ...
    function initialize(    
        ...
        _sky = ISky(IVotingEscrow(__ve).token());
        ...
        weekly = 2_600_000 * 1e18; // represents a starting weekly emission of 2.6M SKY (SKY has 18 decimals)
        isFirstMint = true;
    ...
    function _initialize(
        ...
        if(max > 0){
            _sky.mint(address(this), max);
            _sky.approve(address(_ve), type(uint).max);
    ...
    function circulating_supply() public view returns (uint) {
        return _sky.totalSupply() - _sky.balanceOf(address(_ve));
    }
    ...
    function calculate_rebase(uint _weeklyMint) public view returns (uint) {
        uint _veTotal = _sky.balanceOf(address(_ve));
        uint _skyTotal = _sky.totalSupply();
        
        uint lockedShare = (_veTotal) * PRECISION  / _skyTotal;
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) { // only trigger if new week
            ...
            uint _balanceOf = _sky.balanceOf(address(this));
            if (_balanceOf < _required) {
                _sky.mint(address(this), _required - _balanceOf);
            }

            require(_sky.transfer(team, _teamEmissions));
            
            require(_sky.transfer(address(_rewards_distributor), _rebase));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _sky.approve(address(_voter), _gauge);

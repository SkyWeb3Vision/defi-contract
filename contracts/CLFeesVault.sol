...
contract CLFeesVault {
    ...
    constructor(address _pool, address _permissionRegistry, address _voter, address _gammaFeeRecipient) {
        ...
        pool = _pool;
        voter = IVoter(_voter);
        smpStakingConverter = IPairFactory(pairFactoryClassic).stakingFeeHandler();
    ...
    function claimFees() external onlyGauge returns(uint256 gauge0, uint256 gauge1) {
        ...
        if(_amount0 > 0){
            ...
            if(nft > 0) IERC20(t0).safeTransfer(smpStakingConverter, nft);
        ...
        if(_amount1 > 0){
            ...
            if(nft > 0) IERC20(t1).safeTransfer(smpStakingConverter, nft);
    ...
    function _getFees(uint amount) internal view returns(uint gamma, uint referral, uint nft, uint gauge) {   
        ...
        uint256 smpFee = IPairFactory(pairFactoryClassic).stakingNFTFee();
        referral = amount * referralFee / PRECISION;
        nft = (amount - referral) * smpFee / PRECISION;
    ...
    function setNftStaking(address _smpStaking) external onlyAdmin {
        require(_smpStaking != address(0));
        smpStakingConverter = _smpStaking;
    }
    ...
    function emergencyRecoverERC20(address tokenAddress, uint256 tokenAmount) external onlyAdmin {
        ...
        IERC20(tokenAddress).safeTransfer(permissionsRegsitry.skyTeamMultisig(), tokenAmount);
    }
